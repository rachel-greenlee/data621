...
# Introduction
This analysis predicts the probability that a given insurance customer will be involved in a car crash, as well as the cost of the crash to the insurance company. We begin with an exploratory analysis of the data to guide our preprocessing steps. We then construct two models types: a logistic regression for the binary target variable of Crash vs No Crash; and a least-squares model for the target dollar cost variable.  Ultimately, we will integrate both results to help the insurer evaluate their financial risk.
...
We identified and removed one observation for car_age which had a negative value. 
...
To complement our review of variable distributions we evaluated the overall count and proportion of outliers for data within each variable. The presence and general structure (e.g., clumping) of outliers can point unique circumstances that lend insight the observations. They also provide a basis for identifying data points that may exert high leverage and influence with regard to model fit. 
...
num_box<-num_box %>% dplyr::select(-target_amt)
...
ggarrange(plotlist=b_plots, height = .25, width = .25 , ncol = 3)
...
df_num = df%>%select_if( is.numeric)%>%names()
for (v in seq(1,length(df_num))) {
...
```{r plot covariance, fig.height=9, fig.width=9, fig.align = 'center'}
df%>%plot_correlate() 
```
...
```{r flagm1_diagnostics, fig.height = 9, fig.width = 9, fig.align= 'center'}
#roc curve with AUC
...
Resid_DF = round(deviance(model1)/df.residual(model1),4) # if considerably greater than one we should be concerned
...
Chi_sq = round(devchi,4)
cbind(Resid_DF,Chi_sq)%>%data.frame()%>%flextable()
```
...
```{r flagm3_dispersion}
# evaluate using deviance and quasibinomial comparison
Resid_DF = deviance(model3)/df.residual(model3) # if considerably greater than one we should be concerned
...
We circle back to the test subset that we partitioned earlier.  The below confirms the performance for model2 and we can move forward with this as our select.

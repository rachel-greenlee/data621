...
We calculate the dispersion parameter that's based on Pearson's Chi-squared statistic and the degrees of freedom with a function from the AER library.. A value over 1 indicates over-dispersion, which in this case we do not find as we have a value of 0.897. Generally a value greater than 1.10 is considering dispersed, so we extend this rational to say a value of nearl 0.9 is close enough to 1.0 to not be concerned about dispersion issues. 


```{r}
dispersiontest(m_pois_1)

# sum(residuals(m_pois_1,type ="pearson")^2)/m_pois_1$df.residual ## will get same value from this
```


Plotting the estimated variance against the mean for the above model.
###### not sure how to interpret this, I think we want ideally we want the data balance around the line - rachel

```{r}
plot(log(fitted(m_pois_1)),log((raw$TARGET-fitted(m_pois_1))^2),xlab=expression(hat(mu)),ylab=expression((y-hat(mu))^2),pch=20,col="blue")
abline(0,1)
```
...
For our second poisson model we include all variables, except for the original `STARS` rating. A dispersion check yields a 0.895 value, which is not cause for concern.
...
For our final poisson model we use backwards selection on the above model to see if we can simplify the model. A dispersion value of 0.951 is not a concern.  
...
m_pois_3 <- step(m_pois_2, selection='backwards')
summary(m_pois_3)
```


```{r}
dispersiontest(m_pois_3)
```
...
Poisson Model #1 had an AIC of 45734.  
Poisson Model #2 had an AIC of 45720.
Poisson Model #3 had an AIC of 45717.  

These are all very similar, but Poisson Model #3 that used backwards elimination on the fullest model has the best AIC score. 

### The Negative Binomials

Generally the Negative Binomial is used in favor of the Poisson when the response variable is a count but the mean does not equal the variance. Further if there is overdispersion a Negative Binomial should be used instead of a Poisson.  
...
The AIC of 45736 is extremely close to the parallel Poisson model which had an AIC of 45734.

```{r}
m_binom_1 <- glm.nb(TARGET ~ AcidIndex + STARS_new-1 + LabelAppeal_factor, raw)
summary(m_binom_1)
```
...
This is the lowest AIC we've see so far, at 45722.
...
For the third Negative Binomial we perform backwards selection on the above binomial model. This results in the lowest AIC yet at 45720.

```{r}
m_binom_3 <- step(m_binom_2, selection='backwards')
summary(m_binom_3)
```
...
First, we try a model with the 3 variables we started with in the Poisson and Negative Binomial sections, `AcidIndex`, `STARS_new`, and `LabelAppeal_factor`. The Adjusted R-squared of 0.533 means that the model can explain 53.3% of the varirance in the data.
...
m_mlin_2 <- step(m_mlin_full, selection='backwards')
summary(m_mlin_2)
```

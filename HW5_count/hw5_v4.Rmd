...
For our final poisson model we use backwards selection on the above model to see if we can simplify the model. 
...
In look at models 1 & 3 from the Poisson and Negative Binomials, we see very similar accuracy ratings when tested against our original dataset, all around 29%. A plot of the first Poisson model shows less accuracy at the extreme ends of the 0-8 cases bought range, which makes sense.  

**We choose to move forward with the first Poisson model, as theoretically a Poisson model is more appropriate than a Negative Binomial and because it is easiest to interpret than the backward selection models. This model is based on the `AcidIndex`, `STAR_new`, and `LabelAppeal_factor` variables.**


```{r}
raw$m_pois_1 <- round(predict(m_pois_1,raw,type='response'))
raw$m_pois_3 <- round(predict(m_pois_3,raw,type='response'))
raw$m_binom_1 <- round(predict(m_binom_1,raw,type='response'))
raw$m_binom_3 <- round(predict(m_binom_3,raw,type='response'))
```
...
cm_binom_1 <- confusionMatrix(as.factor(raw$m_binom_1),as.factor(raw$TARGET))
cm_binom_1$overall['Accuracy']

cm_binom_3 <- confusionMatrix(as.factor(raw$m_binom_3),as.factor(raw$TARGET))
cm_binom_3$overall['Accuracy']
...
```{r}
raw$m_pois_1_color <- ifelse(raw$m_pois_1!=raw$TARGET,'wrong','correct')

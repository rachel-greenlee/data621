...
First we want to split the data into training and test data sets, using about 80% to train and 20% to test.
```{r}
set.seed(1)
trainRows <- sample(nrow(raw),as.integer(nrow(raw)*0.80))
train_df = raw[trainRows,]
test_df = raw[-trainRows,]
```
...
### The Poissons
```{r}
m_pois_1 <- glm(TARGET ~ AcidIndex + STARS_new + LabelAppeal_factor, family = "poisson", train_df)

## all variables, except original STARS (so it doesn't drop NAs)
m_pois_2 <- glm(TARGET ~ ., family = "poisson", train_df[,c(1,3,5:13)])
...
```
```{r}
plot(log(fitted(m_pois_2)),log((train_df$TARGET-fitted(m_pois_2))^2),xlab=expression(hat(mu)),ylab=expression((y-hat(mu))^2),pch=20)
abline(0,1,col="red")
```
...
For our final poisson model we use backwards selection on the above model to see if we can simplify the model. A dispersion value of `r round(dispersiontest(m_pois_3),3)`` is not a concern.  
...
Poisson Model #1 had an AIC of `r round(m_pois_1$aic,0)`.  
...
```{r}
m_binom_1 <- glm.nb(TARGET ~ AcidIndex + STARS_new + LabelAppeal_factor, train_df)

m_binom_2 <- glm.nb(TARGET ~ ., train_df[,c(1,3,5:13)])
...
```{r}
m_mlin_1 <- lm(TARGET ~ AcidIndex + STARS_new + LabelAppeal_factor, train_df)
...
```{r}
m_mlin_full <- lm(TARGET ~ ., train_df[,c(1,3,5:13)])
...
```{r}
test_df$m_pois_1 <- round(predict(m_pois_1,test_df,type='response'))
```
```{r}
test_df$m_pois_1_color <- ifelse(test_df$m_pois_1!=test_df$TARGET,'wrong','correct')

test_df %>%
  ggplot(aes(TARGET,m_pois_1,color=m_pois_1_color))+
  geom_jitter()+
  theme_classic()
```
```{r}
confusionMatrix(as.factor(test_df$m_pois_1),as.factor(test_df$TARGET))
```
